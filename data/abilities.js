...
exports.BattleAbilities = {
	...
	"butterflyeffect": {
		...
		onResidual: function(pokemon){
			if(pokemon.removeVolatile('butterflyeffect')){
				var stats = [], i = '';
				var boost = {};
				var special = false, physical = false;
				for (let j = 0; j < pokemon.moveset.length; j++) {
					let move = this.getMove(pokemon.moveset[j].move);
					if (move.category === 'Special' && !move.ohko) {
						special = true;
					} else if (move.category === 'Physical' && !move.ohko) {
						physical = true;
					}
				}
				for (var i in pokemon.boosts) {
					if (pokemon.boosts[i] < 6 && !(i in {accuracy:1, evasion:1})) {
						if (!special && i in {spa: 1}) continue;
						if (!physical && i in {atk: 1}) continue;
						stats.push(i);
					}
				}
				if (stats.length) {
				...
				if(r < 50){
					...
					for (var j in pokemon.boosts) {
						if (pokemon.boosts[j] < 6 && !(j in {accuracy:1, evasion:1})) {
							if (!special && j in {spa: 1}) continue;
							if (!physical && j in {atk: 1}) continue;
							stats.push(j);
						}
					}
					if (stats.length) {
					...
					if(r < 25){
						...
						for (var k in pokemon.boosts) {
							if (pokemon.boosts[k] < 6 && !(k in {accuracy:1, evasion:1})) {
								if (!special && k in {spa: 1}) continue;
								if (!physical && k in {atk: 1}) continue;
								stats.push(k);
							}
						}
						if (stats.length) {
						...
						if(Math.random()*100 < 1){
							...
							for (var x in pokemon.boosts) {
								if (pokemon.boosts[x] < 6 && !(x in {accuracy:1, evasion:1})) {
									if (!special && x in {spa: 1}) continue;
									if (!physical && x in {atk: 1}) continue;
									stats.push(x);
								}
							}
							if (stats.length) {
	...
	"purepower": {
		shortDesc: "This Pokemon's Attack is 100% higher.",
		onModifyAtkPriority: 5,
		onModifyAtk: function (atk) {
			return this.chainModify(2);
		},
